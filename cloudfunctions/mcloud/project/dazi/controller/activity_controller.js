...
class ActivityController extends BaseProjectController {
	...
	async getActivityList() {
		...
		let rules = {
			type: 'string|must',
			cateId: 'string',
			search: 'string|min:1|max:30|name=搜索条             件',
		...
		for (let k = 0; k < list.length; k++) {
			...
			let week = timeUtil.week(timeUtil.timestamp2Time(list[k].ACTIVITY_START, 'Y-M-D'));
			list[k].start = timeUtil.timestamp2Time(list[k].ACTIVITY_START, 'M月D日 h:m') + ' ' + week;
